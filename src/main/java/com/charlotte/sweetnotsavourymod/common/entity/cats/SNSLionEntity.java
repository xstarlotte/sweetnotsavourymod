...
public class SNSLionEntity extends TamableAnimal implements IAnimatable, IVariable<LionVariant> {
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		if (event.isMoving()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.lion.running", ILoopType.EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
		if (this.isSitting()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.lion.sitting", ILoopType.EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
		event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.lion.idle", ILoopType.EDefaultLoopTypes.LOOP));
		return PlayState.CONTINUE;
	}
...
//breeding
	...
	public boolean canMate(Animal mate) {
		if (mate == this) {
			...
			return true;
		} else if (!(mate instanceof com.charlotte.sweetnotsavourymod.common.entity.cats.SNSLionEntity)) {
			return false;
		} else {
			com.charlotte.sweetnotsavourymod.common.entity.cats.SNSLionEntity mob = (com.charlotte.sweetnotsavourymod.common.entity.cats.SNSLionEntity)mate;
			if (!mob.isTame()) {
				return true;
