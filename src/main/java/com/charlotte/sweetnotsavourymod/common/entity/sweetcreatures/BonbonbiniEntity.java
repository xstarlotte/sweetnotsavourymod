...
public class BonbonbiniEntity extends TamableAnimal implements IAnimatable, IVariable<BonbonbiniVariant> {
	...
	public BonbonbiniEntity(EntityType<? extends TamableAnimal> type, Level worldIn) {
		super(type, worldIn);
		setTame(false);
		this.noCulling = true;
	}
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		if (event.isMoving()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bonbonbini.running", ILoopType.EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
		if (this.isSitting()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bonbonbini.sitting", ILoopType.EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
		event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bonbonbini.idle", ILoopType.EDefaultLoopTypes.LOOP));
		return PlayState.CONTINUE;
	}
...
//breeding
	...
	public boolean canMate(Animal mate) {
		if (mate == this) {
			...
			return true;
		} else if (!(mate instanceof BonbonbiniEntity)) {
			return false;
		} else {
			BonbonbiniEntity mob = (BonbonbiniEntity)mate;
			if (!mob.isTame()) {
				return true;
	...
	@Override
	public void setVariant(BonbonbiniVariant variant) {
