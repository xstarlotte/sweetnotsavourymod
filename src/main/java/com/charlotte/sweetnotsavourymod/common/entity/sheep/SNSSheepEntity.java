...
public class SNSSheepEntity extends TameableEntity implements IAnimatable {

	private AnimationFactory factory = new AnimationFactory(this);

	private static final DataParameter<Integer> DATA_ID_TYPE_VARIANT =
			EntityDataManager.defineId(com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSSheepEntity.class, DataSerializers.INT);
	private static final DataParameter<Boolean> SITTING =
			EntityDataManager.defineId(com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSSheepEntity.class, DataSerializers.BOOLEAN);

	public SNSSheepEntity(EntityType<? extends TameableEntity> type, World worldIn) {
		super(type, worldIn);
		setTame(false);
		this.noCulling = true;
	}
...
//breeding
	...
	public AgeableEntity getBreedOffspring(ServerWorld serverLevel, AgeableEntity ageablemob) {
		com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSSheepEntity mob = EntityTypesInit.SNSSHEEP.get().create(serverLevel);
	...
	public boolean canMate(AnimalEntity mate) {
		if (mate == this) {
			...
			return true;
		} else if (!(mate instanceof com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSSheepEntity)) {
			return false;
		} else {
			com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSSheepEntity mob = (com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSSheepEntity)mate;
			if (!mob.isTame()) {
				return true;
	...
	public ILivingEntityData finalizeSpawn(IServerWorld p_146746_, DifficultyInstance p_146747_,
		...
		SheepVariant variant = Util.getRandom(SheepVariant.values(), this.random);
		setVariant(variant);
