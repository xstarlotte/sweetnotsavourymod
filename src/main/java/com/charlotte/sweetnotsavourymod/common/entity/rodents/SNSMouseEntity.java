...
public class SNSMouseEntity extends TameableEntity implements IAnimatable {

	private AnimationFactory factory = new AnimationFactory(this);

	private static final DataParameter<Integer> DATA_ID_TYPE_VARIANT =
			EntityDataManager.defineId(com.charlotte.sweetnotsavourymod.common.entity.rodents.SNSMouseEntity.class, DataSerializers.INT);
	private static final DataParameter<Boolean> SITTING =
			EntityDataManager.defineId(com.charlotte.sweetnotsavourymod.common.entity.rodents.SNSMouseEntity.class, DataSerializers.BOOLEAN);

	public SNSMouseEntity(EntityType<? extends TameableEntity> type, World worldIn) {
		super(type, worldIn);
		setTame(false);
		this.noCulling = true;
	}
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		if (event.isMoving()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.mouse.running", true));
			return PlayState.CONTINUE;
		}
		if (this.isSitting()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.mouse.sitting", true));
			return PlayState.CONTINUE;
		}
		event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.mouse.idle", true));
		return PlayState.CONTINUE;
	}
...
//breeding
	...
	public AgeableMob getBreedOffspring(ServerWorld serverLevel, AgeableMob ageablemob) {
		com.charlotte.sweetnotsavourymod.common.entity.rodents.SNSMouseEntity mob = EntityTypesInit.SNSMOUSE.get().create(serverLevel);
	...
	public boolean canMate(Animal mate) {
		if (mate == this) {
			...
			return true;
		} else if (!(mate instanceof com.charlotte.sweetnotsavourymod.common.entity.rodents.SNSMouseEntity)) {
			return false;
		} else {
			com.charlotte.sweetnotsavourymod.common.entity.rodents.SNSMouseEntity mob = (com.charlotte.sweetnotsavourymod.common.entity.rodents.SNSMouseEntity)mate;
			if (!mob.isTame()) {
				return true;
	...
	public SpawnGroupData finalizeSpawn(IServerWorld p_146746_, DifficultyInstance p_146747_,
		...
		MouseFlavourVariant variant = Util.getRandom(MouseFlavourVariant.values(), this.random);
		setVariant(variant);
		return super.finalizeSpawn(p_146746_, p_146747_, p_146748_, p_146749_, p_146750_);
	}

	private void setVariant(MouseFlavourVariant variant) {
		this.entityData.set(DATA_ID_TYPE_VARIANT, variant.getId() & 255);
	}

	public MouseFlavourVariant getVariant() {
		return MouseFlavourVariant.byId(this.getTypeVariant() & 255);
	}
