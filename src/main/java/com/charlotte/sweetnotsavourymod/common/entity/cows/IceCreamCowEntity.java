...
public class IceCreamCowEntity extends TameableEntity implements IAnimatable {

    private AnimationFactory factory = new AnimationFactory(this);

    private static final DataParameter<Integer> DATA_ID_TYPE_VARIANT =
            EntityDataManager.defineId(com.charlotte.sweetnotsavourymod.common.entity.cows.IceCreamCowEntity.class, DataSerializers.INT);
    private static final DataParameter<Boolean> SITTING =
            EntityDataManager.defineId(com.charlotte.sweetnotsavourymod.common.entity.cows.IceCreamCowEntity.class, DataSerializers.BOOLEAN);

    public IceCreamCowEntity(EntityType<? extends TameableEntity> type, World worldIn) {
        super(type, worldIn);
        setTame(false);
        this.noCulling = true;
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.icecreamcow.running", true));
            return PlayState.CONTINUE;
        }
        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.icecreamcow.sitting", true));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.icecreamcow.idle", true));
        return PlayState.CONTINUE;
    }
