...
public class IceCreamCowEntity extends TamableAnimal implements IAnimatable, IVariable<IceCreamCowVariant> {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(com.charlotte.sweetnotsavourymod.common.entity.cows.IceCreamCowEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> SITTING =
            SynchedEntityData.defineId(com.charlotte.sweetnotsavourymod.common.entity.cows.IceCreamCowEntity.class, EntityDataSerializers.BOOLEAN);

    public IceCreamCowEntity(EntityType<? extends TamableAnimal> type, Level worldIn) {
        super(type, worldIn);
        setTame(false);
        this.noCulling = true;
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.icecreamcow.running", ILoopType.EDefaultLoopTypes.LOOP));
            return PlayState.CONTINUE;
        }
        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.icecreamcow.sitting", ILoopType.EDefaultLoopTypes.LOOP));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.icecreamcow.idle", ILoopType.EDefaultLoopTypes.LOOP));
        return PlayState.CONTINUE;
    }
...
//breeding
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob ageablemob) {
        com.charlotte.sweetnotsavourymod.common.entity.cows.IceCreamCowEntity mob = EntityTypesInit.ICECREAMCOW.get().create(serverLevel);
    ...
    public boolean canMate(Animal mate) {
        if (mate == this) {
            ...
            return true;
        } else if (!(mate instanceof com.charlotte.sweetnotsavourymod.common.entity.cows.IceCreamCowEntity)) {
            return false;
        } else {
            com.charlotte.sweetnotsavourymod.common.entity.cows.IceCreamCowEntity mob = (com.charlotte.sweetnotsavourymod.common.entity.cows.IceCreamCowEntity)mate;
            if (!mob.isTame()) {
                return true;
    ...
    @Override
    public void setVariant(IceCreamCowVariant variant) {
