...
public class LemonParrotEntity extends TameableEntity implements IFlyingAnimal {
	private static final Predicate<MobEntity> CAN_MIMIC = new Predicate<MobEntity>() {
	      public boolean test(@Nullable MobEntity p_test_1_) {
	         return p_test_1_ != null && LemonParrotEntity.IMITATION_SOUND_EVENTS.containsKey(p_test_1_.getType());
	      }
	   };
	   private static final Item DEADLY_ITEM = Items.PUFFERFISH;
	   ...
	   public LemonParrotEntity(EntityType<? extends TameableEntity> type, World worldIn) {
			super(type, worldIn);	
