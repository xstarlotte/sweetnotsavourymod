...
public class SNSBoarryEntity extends TamableAnimal implements IAnimatable, IVariable<BoarryVariant> {
	...
	private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
			SynchedEntityData.defineId(com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSBoarryEntity.class, EntityDataSerializers.INT);
	private static final EntityDataAccessor<Boolean> SITTING =
			SynchedEntityData.defineId(com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSBoarryEntity.class, EntityDataSerializers.BOOLEAN);

	public SNSBoarryEntity(EntityType<? extends TamableAnimal> type, Level worldIn) {
		super(type, worldIn);
		setTame(false);
		this.noCulling = true;
	}
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		if (event.isMoving()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.boarry.running", ILoopType.EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
		if (this.isSitting()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.boarry.sitting", ILoopType.EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
		event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.boarry.idle", ILoopType.EDefaultLoopTypes.LOOP));
		return PlayState.CONTINUE;
	}
...
//breeding
	...
	public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob ageablemob) {
		com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSBoarryEntity mob = EntityTypesInit.SNSBOARRY.get().create(serverLevel);
	...
	public boolean canMate(Animal mate) {
		if (mate == this) {
			...
			return true;
		} else if (!(mate instanceof com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSBoarryEntity)) {
			return false;
		} else {
			com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSBoarryEntity mob = (com.charlotte.sweetnotsavourymod.common.entity.sheep.SNSBoarryEntity)mate;
			if (!mob.isTame()) {
				return true;
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor p_146746_, DifficultyInstance p_146747_,
		...
		BoarryVariant variant = Util.getRandom(BoarryVariant.values(), this.random);
		setVariant(variant);
		return super.finalizeSpawn(p_146746_, p_146747_, p_146748_, p_146749_, p_146750_);
	}

	@Override
	public void setVariant(BoarryVariant variant) {
		this.entityData.set(DATA_ID_TYPE_VARIANT, variant.getId() & 255);
	}

	@Override
	public BoarryVariant getVariant() {
		return BoarryVariant.byId(this.getTypeVariant() & 255);
	}
