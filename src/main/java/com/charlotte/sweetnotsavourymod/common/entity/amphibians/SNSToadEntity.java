package com.charlotte.sweetnotsavourymod.common.entity.amphibians;
...
public class SNSToadEntity extends TameableEntity implements IAnimatable {

	private AnimationFactory factory = new AnimationFactory(this);

	private static final DataParameter<Integer> DATA_ID_TYPE_VARIANT =
			EntityDataManager.defineId(SNSToadEntity.class, DataSerializers.INT);
	private static final DataParameter<Boolean> SITTING =
			EntityDataManager.defineId(SNSToadEntity.class, DataSerializers.BOOLEAN);

	public SNSToadEntity(EntityType<? extends TameableEntity> type, World worldIn) {
		super(type, worldIn);
		setTame(false);
		this.noCulling = true;
	}
...
//animations
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		if (event.isMoving()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.toad.running", true));
			return PlayState.CONTINUE;
		}
		if (this.isSitting()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.toad.sitting", true));
			return PlayState.CONTINUE;
		}
...
//breeding
	...
	public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob ageablemob) {
		SNSToadEntity mob = EntityTypesInit.SNSTOAD.get().create(serverLevel);
	...
	public boolean canMate(Animal mate) {
		if (mate == this) {
			...
			return true;
		} else if (!(mate instanceof SNSToadEntity)) {
			return false;
		} else {
			SNSToadEntity mob = (SNSToadEntity)mate;
			if (!mob.isTame()) {
				return true;
...
//variants
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor p_146746_, DifficultyInstance p_146747_,
		...
		ToadFlavourVariant variant = Util.getRandom(ToadFlavourVariant.values(), this.random);
		setVariant(variant);
		return super.finalizeSpawn(p_146746_, p_146747_, p_146748_, p_146749_, p_146750_);
	}

	private void setVariant(ToadFlavourVariant variant) {
		this.entityData.set(DATA_ID_TYPE_VARIANT, variant.getId() & 255);
	}

	public ToadFlavourVariant getVariant() {
		return ToadFlavourVariant.byId(this.getTypeVariant() & 255);
	}
